...
pub enum IslandMode {
    Dashboard, 
    Music,
    Notification, // The ephemeral pop-up
    NotificationCenter, // The scrollable history view
    Osd,
}
...
pub enum Message {
    ...
    ClearAllNotifications, // New: Clear history
...
pub struct DynamicIsland {
    ...
    pub notification_info: Option<NotificationData>, // For the current pop-up
    pub notification_history: Vec<NotificationData>, // New: History list
...
impl DynamicIsland {
    pub fn new() -> (Self, Task<Message>) {
        (
            Self {
                ...
                notification_history: Vec::new(), // Init empty
    ...
    pub fn subscription(&self) -> Subscription<Message> {
        ...
        // --- FIX: Only enable Volume Scroll if NOT in Notification Center ---
        // This allows the Scrollable widget to consume the mouse wheel events 
    ...
    fn handle_tick(&mut self) -> Task<Message> {
        ...
        let running_music = self.music_progress.update(dt); 
        let running_osd = self.osd_progress.update(dt);
        ...
        match self.mode {
            ...
            IslandMode::Dashboard => {
                ...
                if self.height.current < 50.0 {
                    ...
                    let extra_w = if !self.notification_history.is_empty() { 20.0 + 15.0 } else { 0.0 }; // Bell width + spacing
    ...
    fn handle_cycle_mode(&mut self) -> Task<Message> {
        match self.mode {
            // Clicking OSD or Pop-up dismisses them
            IslandMode::Osd | IslandMode::Notification => { self.handle_auto_close(); },
            
            // Clicking Music -> Dashboard
            IslandMode::Music => { self.close_to_dashboard(); },
            
            // Clicking Notification Center -> Music (NEW ROUTE)
            IslandMode::NotificationCenter => { 
                self.mode = IslandMode::Music;
                self.height.set_target(config::size::MUSIC_HEIGHT);
                
                // Recalculate width for Music mode
            ...
            // Clicking Dashboard -> Notification Center (if any) OR Music
            IslandMode::Dashboard => {
                if !self.notification_history.is_empty() {
                    ...
                    // No notifications, go straight to Music
                    self.height.set_target(config::size::DASHBOARD_HEIGHT); // Reset height logic first
                    self.mode = IslandMode::Music;
    ...
    fn handle_media_update(&mut self, info: Option<MediaInfo>) -> Task<Message> {
        ...
        if new_info.is_playing && !self.last_known_playing {
            // Only switch to Music automatically if we are in Dashboard
